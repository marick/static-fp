module Choose.KeyedTest exposing (..)

import Test exposing (..)
import TestBuilders exposing (..)
import Choose.Keyed as Keyed
import Choose.Common as Keyed
import Choose.Definitions as D
import Dict

accessors partChooser =
  ( Keyed.get partChooser
  , Keyed.set partChooser
  , Keyed.update partChooser
  )

operations : Test
operations =
  let
    (get, set, update) = accessors D.dictKeyed
  in
    describe "operations" 
...
laws =
  describe "laws for optionals (Keyed)"
    [ lawTests D.dictKeyed
               (D.dict1 "key" "focus")                       "Dict"
        
    , lawTests (Keyed.keyedDict "one" |> Keyed.next (Keyed.keyedDict "two"))
...
compositionHasQuirksInSet =
  let
    composed = Keyed.keyedDict "one" |> Keyed.next (Keyed.keyedDict "two")
